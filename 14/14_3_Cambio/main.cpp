...
void solve_cambio() {
    ...
    std::vector<int> Csum(M);
    const std::vector<char> chars = { 'A', 'C', 'G', 'T' };
    for (auto ch : chars) {
        S.resize(2 * M);
        for (size_t i = 0; i < M; ++i) {
            Sdigits[i] = sS[i] == ch;
            Pdigits[M - i - 1] = sP[i] == ch;
        }
        std::copy(Sdigits.begin(), Sdigits.begin() + M, Sdigits.begin() + M);
        auto& C = S;
        C *= P;
        auto& Cdigits = C.digits;
        for (size_t k = 0; k < M; ++k) {
            Csum[k] += Cdigits[M - 1 + k];
        }
    }
    size_t k = 0;
    size_t likelihood = Csum[k];
    for (size_t i = 0; i < M; ++i) {
        if (Csum[i] > likelihood) {
            likelihood = Csum[i];
            k = i;
        }
    }
    std::cout << likelihood << " " << k;
}
